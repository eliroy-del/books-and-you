-- Catalog: Golden Writing Book KG 2
...
select
  'golden-writing-book-kg-2',
  'Golden Writing Book KG 2',
  'Kindergarten handwriting practice book',
  'Golden Writing Book KG 2 is a kindergarten handwriting workbook that helps young learners practise letter formation, pencil control, and early writing skills through guided tracing and writing exercises.',
  'Part of the Golden series for early years. This KG 2 title supports classroom and home handwriting practice with child-friendly pages.',
  '978-9988-2-7002-5',
  64,
  'English',
  '2024-01-01',
  p.id,
  '/covers/golden-writing-book-kg-2/front.jpg',
  'from-[#0F766E] via-[#F97316] to-[#EC4899]',
  '#FB923C',
  array['Handwriting','Kindergarten','Writing','Early Years'],
  array['Pencil Control','Letter Tracing','Word Practice','Writing Exercises'],
  true,
  true,
  jsonb_build_object('series','Golden','level','KG 2','subject','Writing')
...
where b.slug = 'golden-writing-book-kg-2'
...
where b.slug = 'golden-writing-book-kg-2'
  and c.slug in ('nursery-kindergarten', 'nursery-handwriting-books', 'level-kindergarten')
...
select b.id, t.tag
from public.books b
cross join (values ('handwriting'), ('writing'), ('kindergarten'), ('kg-2'), ('golden'), ('ghana')) as t(tag)
where b.slug = 'golden-writing-book-kg-2'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-writing-book-kg-2');
...
cross join (
  values
    ('/covers/golden-writing-book-kg-2/front.jpg', 'Golden Writing Book KG 2 front cover', 0, true)
