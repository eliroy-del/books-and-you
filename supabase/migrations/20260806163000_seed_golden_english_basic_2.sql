-- Catalog: Golden English Basic 2 (NaCCA Primary English)
...
select
  'golden-english-basic-2',
  'Golden English Basic 2',
  'Oral Language · Reading · Writing · Grammar',
  'Golden English Basic 2 is a primary school English language textbook based on the NaCCA syllabus. It covers oral language, reading, writing, and grammar to help pupils succeed in communication.',
  'Developed in accordance with the English syllabus issued by the National Council for Curriculum and Assessment (NaCCA) for Primary School. This Golden series title helps Basic 2 pupils develop listening, speaking, reading, and writing skills.',
  '978-9988-2-6613-4',
  140,
  'English',
  '2024-01-01',
  p.id,
  '/covers/golden-english-basic-2/front.jpg',
  'from-[#EA580C] via-[#FBBF24] to-[#0F172A]',
  '#F59E0B',
  array['English Language','Primary 2','Textbooks','NaCCA'],
  array['Oral Language','Reading','Writing','Grammar'],
  true,
  true,
  jsonb_build_object('series','Golden','level','Basic 2','curriculum','NaCCA','subject','English Language')
...
where b.slug = 'golden-english-basic-2'
...
where b.slug = 'golden-english-basic-2'
  and c.slug in ('primary-school', 'primary-english-language', 'level-primary-2')
...
select b.id, t.tag
from public.books b
cross join (values ('english'), ('primary-2'), ('nacca'), ('golden'), ('textbook'), ('ghana')) as t(tag)
where b.slug = 'golden-english-basic-2'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-english-basic-2');
...
cross join (
  values
    ('/covers/golden-english-basic-2/front.jpg', 'Golden English Basic 2 front cover', 0, true),
    ('/covers/golden-english-basic-2/angle.jpg', 'Golden English Basic 2 angled view', 1, false)
