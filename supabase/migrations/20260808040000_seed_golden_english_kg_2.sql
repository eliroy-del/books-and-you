...
select
  ...
  '2024-01-01',
  p.id,
  '/covers/golden-english-kg-2/front.jpg',
  'from-[#0F766E] via-[#F97316] to-[#DC2626]',
  '#14B8A6',
  array['English','Kindergarten','Literacy','Early Years'],
  array['Letters and Sounds','Vocabulary','Reading Practice','Speaking and Listening'],
  true,
  true,
  jsonb_build_object('series','Golden','level','KG 2','subject','English','curriculum','NaCCA')
...
where b.slug = 'golden-english-kg-2'
...
where b.slug = 'golden-english-kg-2'
  and c.slug in ('nursery-kindergarten', 'nursery-textbooks', 'nursery-phonics-books', 'level-kindergarten')
...
select b.id, t.tag
from public.books b
cross join (values ('english'), ('literacy'), ('kindergarten'), ('kg-2'), ('golden'), ('nacca'), ('ghana')) as t(tag)
where b.slug = 'golden-english-kg-2'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-english-kg-2');
...
cross join (
  values
    ('/covers/golden-english-kg-2/front.jpg', 'Golden English KG 2 front cover', 0, true)
