-- Catalog: Golden Computing Basic 4 (NaCCA Primary ICT)
...
select
  ...
  '2024-01-01',
  p.id,
  '/covers/golden-computing-basic-4/front.jpg',
  'from-[#DC2626] via-[#1D4ED8] to-[#FACC15]',
  '#FDE047',
  array['Computing / ICT','Primary 4','Textbooks','NaCCA'],
  array['Computer Basics','Digital Literacy','Applications','Internet Safety'],
  true,
  true,
  jsonb_build_object('series','Golden','level','Basic 4','curriculum','NaCCA','subject','Computing')
...
where b.slug = 'golden-computing-basic-4'
...
where b.slug = 'golden-computing-basic-4'
  and c.slug in ('primary-school', 'primary-computing-ict', 'level-primary-4')
...
select b.id, t.tag
from public.books b
cross join (values ('computing'), ('ict'), ('primary-4'), ('nacca'), ('golden'), ('textbook'), ('ghana')) as t(tag)
where b.slug = 'golden-computing-basic-4'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-computing-basic-4');
...
cross join (
  values
    ('/covers/golden-computing-basic-4/front.jpg', 'Golden Computing Basic 4 front cover', 0, true),
    ('/covers/golden-computing-basic-4/angle.jpg', 'Golden Computing Basic 4 angled view', 1, false)
