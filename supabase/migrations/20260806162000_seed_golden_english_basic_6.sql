-- Catalog: Golden English Basic 6 (NaCCA Primary English)
...
select
  ...
  '2024-01-01',
  p.id,
  '/covers/golden-english-basic-6/front.jpg',
  'from-[#9A3412] via-[#F59E0B] to-[#0F172A]',
  '#FBBF24',
  array['English Language','Primary 6','Textbooks','NaCCA'],
  array['Comprehension','Grammar','Listening and Speaking','Writing'],
  true,
  true,
  jsonb_build_object('series','Golden','level','Basic 6','curriculum','NaCCA','subject','English Language')
...
where b.slug = 'golden-english-basic-6'
...
select b.id, t.tag
from public.books b
cross join (values ('english'), ('primary-6'), ('nacca'), ('golden'), ('textbook'), ('ghana')) as t(tag)
where b.slug = 'golden-english-basic-6'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-english-basic-6');
...
cross join (
  values
    ('/covers/golden-english-basic-6/front.jpg', 'Golden English Basic 6 front cover', 0, true),
    ('/covers/golden-english-basic-6/angle.jpg', 'Golden English Basic 6 angled view', 1, false)
