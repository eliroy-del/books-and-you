...
select
  ...
  'Golden RME Basic 2 is a primary school Religious and Moral Education textbook based on the new NaCCA syllabus. It helps Basic 2 learners explore faith, values, and responsible living through clear lessons and classroom activities.',
  'Part of the Golden series for primary schools. This Basic 2 RME title supports classroom learning aligned with the NaCCA curriculum.',
  '978-9988-2-7012-4',
  128,
  'English',
  '2024-01-01',
  p.id,
  '/covers/golden-rme-basic-2/front.jpg',
  'from-[#FACC15] via-[#F97316] to-[#DC2626]',
  '#FDE047',
  array['RME','Religious and Moral Education','Primary 2','NaCCA'],
  array['Beliefs and Values','Moral Living','Religious Practices','Community and Responsibility'],
  true,
  true,
  jsonb_build_object('series','Golden','level','Basic 2','subject','RME','curriculum','NaCCA')
...
where b.slug = 'golden-rme-basic-2'
...
where b.slug = 'golden-rme-basic-2'
  and c.slug in ('primary-school', 'primary-religious-and-moral-education-rme', 'level-primary-2')
...
select b.id, t.tag
from public.books b
cross join (values ('rme'), ('religious-and-moral-education'), ('primary-2'), ('golden'), ('nacca'), ('ghana')) as t(tag)
where b.slug = 'golden-rme-basic-2'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-rme-basic-2');
...
cross join (
  values
    ('/covers/golden-rme-basic-2/front.jpg', 'Golden RME Basic 2 front cover', 0, true)
