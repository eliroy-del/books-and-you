-- Catalog: Golden History of Ghana BS 2 (NaCCA Primary History)
...
select
  ...
  '2024-01-01',
  p.id,
  '/covers/golden-history-of-ghana-bs-2/front.jpg',
  'from-[#BE123C] via-[#FACC15] to-[#EA580C]',
  '#DC2626',
  array['History','Primary 2','Textbooks','NaCCA'],
  array['Our Community','People of Ghana','National Life','Everyday History'],
  true,
  true,
  jsonb_build_object('series','Golden','level','BS 2','curriculum','NaCCA','subject','History')
...
where b.slug = 'golden-history-of-ghana-bs-2'
...
select b.id, t.tag
from public.books b
cross join (values ('history'), ('primary-2'), ('nacca'), ('golden'), ('textbook'), ('ghana')) as t(tag)
where b.slug = 'golden-history-of-ghana-bs-2'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-history-of-ghana-bs-2');
...
cross join (
  values
    ('/covers/golden-history-of-ghana-bs-2/front.jpg', 'Golden History of Ghana BS 2 front cover', 0, true),
    ('/covers/golden-history-of-ghana-bs-2/angle.jpg', 'Golden History of Ghana BS 2 angled view', 1, false)
