-- Catalog: Golden Writing Book KG 1
...
select
  ...
  '2024-01-01',
  p.id,
  '/covers/golden-writing-book-kg-1/front.jpg',
  'from-[#F97316] via-[#FBBF24] to-[#EC4899]',
  '#F59E0B',
  array['Handwriting','Kindergarten','Writing','Early Years'],
  array['Pencil Control','Letter Tracing','Word Practice','Writing Exercises'],
  true,
  true,
  jsonb_build_object('series','Golden','level','KG 1','subject','Writing')
...
where b.slug = 'golden-writing-book-kg-1'
...
select b.id, t.tag
from public.books b
cross join (values ('handwriting'), ('writing'), ('kindergarten'), ('kg-1'), ('golden'), ('ghana')) as t(tag)
where b.slug = 'golden-writing-book-kg-1'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-writing-book-kg-1');
...
cross join (
  values
    ('/covers/golden-writing-book-kg-1/front.jpg', 'Golden Writing Book KG 1 front cover', 0, true)
