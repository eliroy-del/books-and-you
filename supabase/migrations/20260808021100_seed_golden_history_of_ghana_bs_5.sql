-- Catalog: Golden History of Ghana BS 5 (NaCCA Primary History)
...
select
  ...
  '2024-01-01',
  p.id,
  '/covers/golden-history-of-ghana-bs-5/front.jpg',
  'from-[#9A3412] via-[#FACC15] to-[#EA580C]',
  '#DC2626',
  array['History','Primary 5','Textbooks','NaCCA'],
  array['Our Nation','People of Ghana','Independence','National Life'],
  true,
  true,
  jsonb_build_object('series','Golden','level','BS 5','curriculum','NaCCA','subject','History')
...
where b.slug = 'golden-history-of-ghana-bs-5'
...
select b.id, t.tag
from public.books b
cross join (values ('history'), ('primary-5'), ('nacca'), ('golden'), ('textbook'), ('ghana')) as t(tag)
where b.slug = 'golden-history-of-ghana-bs-5'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-history-of-ghana-bs-5');
...
cross join (
  values
    ('/covers/golden-history-of-ghana-bs-5/front.jpg', 'Golden History of Ghana BS 5 front cover', 0, true),
    ('/covers/golden-history-of-ghana-bs-5/angle.jpg', 'Golden History of Ghana BS 5 angled view', 1, false)
