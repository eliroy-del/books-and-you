...
select
  ...
  'Golden RME Basic 3 is a primary school Religious and Moral Education textbook based on the new NaCCA syllabus. It helps Basic 3 learners explore faith, values, and responsible living through clear lessons and classroom activities.',
  'Part of the Golden series for primary schools. This Basic 3 RME title supports classroom learning aligned with the NaCCA curriculum.',
  '978-9988-2-7011-7',
  128,
  'English',
  '2024-01-01',
  p.id,
  '/covers/golden-rme-basic-3/front.jpg',
  'from-[#16A34A] via-[#FFFFFF] to-[#DC2626]',
  '#22C55E',
  array['RME','Religious and Moral Education','Primary 3','NaCCA'],
  array['Beliefs and Values','Moral Living','Religious Practices','Community and Responsibility'],
  true,
  true,
  jsonb_build_object('series','Golden','level','Basic 3','subject','RME','curriculum','NaCCA')
...
where b.slug = 'golden-rme-basic-3'
...
where b.slug = 'golden-rme-basic-3'
  and c.slug in ('primary-school', 'primary-religious-and-moral-education-rme', 'level-primary-3')
...
select b.id, t.tag
from public.books b
cross join (values ('rme'), ('religious-and-moral-education'), ('primary-3'), ('golden'), ('nacca'), ('ghana')) as t(tag)
where b.slug = 'golden-rme-basic-3'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-rme-basic-3');
...
cross join (
  values
    ('/covers/golden-rme-basic-3/front.jpg', 'Golden RME Basic 3 front cover', 0, true)
