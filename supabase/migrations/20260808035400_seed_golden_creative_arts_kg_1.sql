-- Catalog: Golden Creative Arts KG 1
...
insert into public.authors (slug, name, nationality, bio, is_verified) values
  ...
  ('okyere-baafi-alexander', 'Okyere Baafi Alexander', 'Ghanaian', 'Author of Golden series textbooks and early-years activity books for Ghanaian schools.', true)
...
select
  'golden-creative-arts-kg-1',
  'Golden Creative Arts KG 1',
  'Based on the new NaCCA syllabus',
  'Golden Creative Arts KG 1 is a kindergarten creative arts textbook based on the new NaCCA syllabus. It introduces young learners to drawing, colour, music, and simple creative activities through age-appropriate lessons.',
  'Part of the Golden series for early years. This KG 1 Creative Arts title supports classroom learning aligned with the NaCCA curriculum.',
  '978-9988-2-7006-3',
  80,
  'English',
  '2024-01-01',
  p.id,
  '/covers/golden-creative-arts-kg-1/front.jpg',
  'from-[#7C3AED] via-[#F97316] to-[#DC2626]',
  '#F97316',
  array['Creative Arts','Kindergarten','Art','Early Years'],
  array['Drawing and Colour','Music and Movement','Craft Activities','Creative Play'],
  true,
  true,
  jsonb_build_object('series','Golden','level','KG 1','subject','Creative Arts','curriculum','NaCCA')
...
select
  ...
  'SKU-GOLDEN-CREATIVE-ARTS-KG1-PB',
  5500,
  null,
  'GHS',
  100,
...
select b.id, t.tag
from public.books b
cross join (values ('creative-arts'), ('art'), ('kindergarten'), ('kg-1'), ('golden'), ('nacca'), ('ghana')) as t(tag)
where b.slug = 'golden-creative-arts-kg-1'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-creative-arts-kg-1');
...
cross join (
  values
    ('/covers/golden-creative-arts-kg-1/front.jpg', 'Golden Creative Arts KG 1 front cover', 0, true)
