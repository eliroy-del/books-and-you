-- Catalog: Golden History of Ghana BS 6 (NaCCA Primary History)
...
select
  ...
  '2024-01-01',
  p.id,
  '/covers/golden-history-of-ghana-bs-6/front.jpg',
  'from-[#7C3AED] via-[#FACC15] to-[#EA580C]',
  '#DC2626',
  array['History','Primary 6','Textbooks','NaCCA'],
  array['Our Nation','Independence','Governance','National Life'],
  true,
  true,
  jsonb_build_object('series','Golden','level','BS 6','curriculum','NaCCA','subject','History')
...
where b.slug = 'golden-history-of-ghana-bs-6'
...
where b.slug = 'golden-history-of-ghana-bs-6'
  and c.slug in ('primary-school', 'primary-history', 'level-primary-6')
...
select b.id, t.tag
from public.books b
cross join (values ('history'), ('primary-6'), ('nacca'), ('golden'), ('textbook'), ('ghana')) as t(tag)
where b.slug = 'golden-history-of-ghana-bs-6'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-history-of-ghana-bs-6');
...
cross join (
  values
    ('/covers/golden-history-of-ghana-bs-6/front.jpg', 'Golden History of Ghana BS 6 front cover', 0, true),
    ('/covers/golden-history-of-ghana-bs-6/angle.jpg', 'Golden History of Ghana BS 6 angled view', 1, false)
