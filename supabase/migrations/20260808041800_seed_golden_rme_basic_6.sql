...
select
  ...
  'Golden RME Basic 6 is a primary school Religious and Moral Education textbook based on the new NaCCA syllabus. It helps Basic 6 learners explore faith, values, and responsible living through clear lessons and classroom activities.',
  'Part of the Golden series for primary schools. This Basic 6 RME title supports classroom learning aligned with the NaCCA curriculum.',
  '978-9988-2-7015-5',
  144,
  'English',
  '2024-01-01',
  p.id,
  '/covers/golden-rme-basic-6/front.jpg',
  'from-[#EA580C] via-[#FFFFFF] to-[#DC2626]',
  '#F97316',
  array['RME','Religious and Moral Education','Primary 6','NaCCA'],
  array['Beliefs and Values','Moral Living','Religious Practices','Community and Responsibility'],
  true,
  true,
  jsonb_build_object('series','Golden','level','Basic 6','subject','RME','curriculum','NaCCA')
...
where b.slug = 'golden-rme-basic-6'
...
where b.slug = 'golden-rme-basic-6'
  and c.slug in ('primary-school', 'primary-religious-and-moral-education-rme', 'level-primary-6')
...
select b.id, t.tag
from public.books b
cross join (values ('rme'), ('religious-and-moral-education'), ('primary-6'), ('golden'), ('nacca'), ('ghana')) as t(tag)
where b.slug = 'golden-rme-basic-6'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-rme-basic-6');
...
cross join (
  values
    ('/covers/golden-rme-basic-6/front.jpg', 'Golden RME Basic 6 front cover', 0, true)
