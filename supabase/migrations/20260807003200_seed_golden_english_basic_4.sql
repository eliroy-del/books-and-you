-- Catalog: Golden English Basic 4 (NaCCA Primary English)
...
select
  ...
  '2024-01-01',
  p.id,
  '/covers/golden-english-basic-4/front.jpg',
  'from-[#B91C1C] via-[#2563EB] to-[#0F172A]',
  '#FBBF24',
  array['English Language','Primary 4','Textbooks','NaCCA'],
  array['Oral Language','Reading','Writing','Grammar'],
  true,
  true,
  jsonb_build_object('series','Golden','level','Basic 4','curriculum','NaCCA','subject','English Language')
...
where b.slug = 'golden-english-basic-4'
...
where b.slug = 'golden-english-basic-4'
  and c.slug in ('primary-school', 'primary-english-language', 'level-primary-4')
...
select b.id, t.tag
from public.books b
cross join (values ('english'), ('primary-4'), ('nacca'), ('golden'), ('textbook'), ('ghana')) as t(tag)
where b.slug = 'golden-english-basic-4'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-english-basic-4');
...
cross join (
  values
    ('/covers/golden-english-basic-4/front.jpg', 'Golden English Basic 4 front cover', 0, true),
    ('/covers/golden-english-basic-4/angle.jpg', 'Golden English Basic 4 angled view', 1, false)
