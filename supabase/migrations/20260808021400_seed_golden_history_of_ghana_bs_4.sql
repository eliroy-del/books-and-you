-- Catalog: Golden History of Ghana BS 4 (NaCCA Primary History)
...
select
  'golden-history-of-ghana-bs-4',
  'Golden History of Ghana BS 4',
  'Based on the New NaCCA Syllabus',
  'Golden History of Ghana BS 4 is a primary school history textbook based on the new NaCCA syllabus. It develops Basic 4 learners'' understanding of Ghana''s story, people, and national life through clear lessons and classroom activities.',
  'Aligned with the National Council for Curriculum and Assessment (NaCCA) syllabus for Primary School. This Golden series title helps Basic 4 pupils explore the history of Ghana with age-appropriate content.',
  '978-9988-2-6904-0',
  148,
  'English',
  '2024-01-01',
  p.id,
  '/covers/golden-history-of-ghana-bs-4/front.jpg',
  'from-[#EA580C] via-[#FACC15] to-[#DC2626]',
  '#BE123C',
  array['History','Primary 4','Textbooks','NaCCA'],
  array['Our Nation','People of Ghana','Independence','National Life'],
  true,
  true,
  jsonb_build_object('series','Golden','level','BS 4','curriculum','NaCCA','subject','History')
...
where b.slug = 'golden-history-of-ghana-bs-4'
...
select b.id, t.tag
from public.books b
cross join (values ('history'), ('primary-4'), ('nacca'), ('golden'), ('textbook'), ('ghana')) as t(tag)
where b.slug = 'golden-history-of-ghana-bs-4'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-history-of-ghana-bs-4');
...
cross join (
  values
    ('/covers/golden-history-of-ghana-bs-4/front.jpg', 'Golden History of Ghana BS 4 front cover', 0, true),
    ('/covers/golden-history-of-ghana-bs-4/angle.jpg', 'Golden History of Ghana BS 4 angled view', 1, false)
