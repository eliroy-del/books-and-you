-- Catalog: Golden English Basic 1 (NaCCA Primary English)
...
select
  ...
  '2024-01-01',
  p.id,
  '/covers/golden-english-basic-1/front.jpg',
  'from-[#1D4ED8] via-[#F97316] to-[#0F172A]',
  '#FBBF24',
  array['English Language','Primary 1','Textbooks','NaCCA'],
  array['Oral Language','Reading','Writing','Grammar'],
  true,
  true,
  jsonb_build_object('series','Golden','level','Basic 1','curriculum','NaCCA','subject','English Language')
...
where b.slug = 'golden-english-basic-1'
...
where b.slug = 'golden-english-basic-1'
  and c.slug in ('primary-school', 'primary-english-language', 'level-primary-1')
...
select b.id, t.tag
from public.books b
cross join (values ('english'), ('primary-1'), ('nacca'), ('golden'), ('textbook'), ('ghana')) as t(tag)
where b.slug = 'golden-english-basic-1'
on conflict do nothing;

delete from public.book_images
where book_id = (select id from public.books where slug = 'golden-english-basic-1');
...
cross join (
  values
    ('/covers/golden-english-basic-1/front.jpg', 'Golden English Basic 1 front cover', 0, true),
    ('/covers/golden-english-basic-1/angle.jpg', 'Golden English Basic 1 angled view', 1, false)
