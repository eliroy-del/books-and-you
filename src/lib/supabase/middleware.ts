...
import { getSupabaseEnv, isDemoAuthAllowed, isSupabaseConfigured } from "@/lib/supabase/env";
...
export async function updateSession(request: NextRequest) {
  ...
  if (!isSupabaseConfigured()) {
    // Without live auth, staff consoles stay locked unless explicit local demo mode is enabled.
    if (isStaffPath && !isDemoAuthAllowed()) {
